...
class TestNewsTier2Crawler:
    ...
    async def test_fetch_raw_data_returns_list(self):
        ...
        with patch("apps.batch.services.news.crawler_tier2_api.asyncio.to_thread", new_callable=AsyncMock, return_value=MagicMock(entries=[])):
            try:
...
class TestNewsTier3Crawler:
    ...
    async def test_fetch_raw_data_returns_list_when_no_db(self):
        ...
        crawler = NewsTier3Crawler()
        # DB 없는 상황을 mock
        with patch("apps.batch.services.news.crawler_tier3_browser.AsyncSessionLocal", None, create=True):
            try:
                result = await crawler.fetch_raw_data()
                assert isinstance(result, list)
            except Exception:
